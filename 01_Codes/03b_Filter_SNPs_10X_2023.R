...
cov.data %>% dplyr::filter(mean_cov_ns < 10) %>% nrow()
...
ID.coverage <- cov.data %>% filter(between(mean_cov_ns, 10, 75)) %>% 
  mutate(Pop = "NoPop") %>% 
  select(sample, Pop) 


write.table(ID.coverage, 
            file = file.path(filter.ref.path, "B_10X_samples", "popmap.coverage_10x.txt"),
            quote = FALSE, sep = "\t",
...
if(!file.exists(file.path(filter.ref.path, "B_10X_samples", "01_r75_MAF01"))){
  dir.create(file.path(filter.ref.path, "B_10X_samples", "01_r75_MAF01"))
  print(file.path(filter.ref.path,"B_10X_samples", "01_r75_MAF01"))
}

cmd <- paste("-P", stacks.ref.path, 
             "-M", file.path(filter.ref.path, "B_10X_samples", "popmap.coverage_10x.txt"),
             "--out-path",  file.path(filter.ref.path, "B_10X_samples",  "01_r75_MAF01"),
             "-t", 8,
             "-r", r.value, #              
             "--min-maf", maf.value,
             #"--smooth",
...
cat("*", "!.gitignore", "*.log", sep = "\n",
    file = file.path(filter.ref.path,  "B_10X_samples",  "01_r75_MAF01", ".gitignore"))
...
filter.stat <- read.delim(file.path(filter.ref.path,  "B_10X_samples",  "01_r75_MAF01", "populations.sumstats.tsv"), 
                          skip=1, sep = "\t", header = T )
...
list.files(file.path(filter.ref.path,  "B_10X_samples",  "01_r75_MAF01"))

vcf.path <- file.path(filter.ref.path,  "B_10X_samples",  "01_r75_MAF01", "populations.snps.vcf")
...
setwd(file.path(filter.ref.path,  "B_10X_samples",  "02_MissingData")) 
...
imiss <- read.delim(file.path(filter.ref.path, "B_10X_samples",  "02_MissingData", "out.imiss"), skip=0, sep = "\t", header = T )
imiss %>% head()
...
graph2.0 <- imiss %>% left_join(pop.data, by = c("INDV" = "ID_GQ")) %>% 
  ggplot(aes(x = Region_echantillonnage, y = F_MISS, col =  F_MISS)) + 
...
ggsave(filename = file.path(filter.ref.path, "B_10X_samples",  "02_MissingData", "NAbyInd_10x_Nov2023.png"),
       plot = graph2.0,
...
cat(file = file.path(filter.ref.path,  "B_10X_samples",  "02_MissingData","VCFtools_RemoveIndMissing.log"),
    "\n", cmd2, "\n",
    A2, # what to put in my file
    append= F, sep = "\n")


list.files(file.path(filter.ref.path,  "B_10X_samples",  "02_MissingData"))

vcf.path <- file.path(filter.ref.path,  "B_10X_samples",  "02_MissingData", "populations.snps.511ind.vcf.recode.vcf")
...
lmiss <- read.delim(file.path(filter.ref.path, "B_10X_samples", "02_MissingData", "out.lmiss"), header = T, sep = "\t" )
...
ggsave(filename = file.path(filter.ref.path,  "B_10X_samples", "02_MissingData", "NAbySNPs_10x_Nov2023.png"),
       plot = graph2.1,
...
cmd4 <- paste("--vcf", file.path(filter.ref.path,  "B_10X_samples", "02_MissingData", "populations.snps.511ind.vcf.recode.vcf"), 
              "--recode",
              "--max-missing", "0.9",
              ...
              "--out", file.path(current.wd, filter.ref.path, "B_10X_samples", "02_MissingData", paste0("populations.", nrow(good.LOC),"snps", ".511ind"))
)
...
cat(file = file.path(filter.ref.path,  "B_10X_samples", "02_MissingData","populations.filt_GLOBAL_10x_b_Remove_Loci.log"),
    "\n", cmd4, "\n",
...
cat("*.vcf", "!.gitignore", sep = "\n",
    file = file.path(filter.ref.path,  "B_10X_samples", "02_MissingData", ".gitignore"))
...
vcf.data <- vcfR::read.vcfR(file.path(filter.ref.path,  "B_10X_samples", "02_MissingData", "populations.178074snps.511ind.recode.vcf"))
...
#     file = file.path(filter.ref.path, "B_10X_samples", "03_HW", "SNPs_10x_HW.RES.RData"))

load(file.path(filter.ref.path,  "B_10X_samples", "03_HW", "SNPs_10x_HW.RES.RData"))
...
ggsave(filename = file.path(filter.ref.path,"B_10X_samples", "03_HW", "He_Ho_Nov2023.png"),
       plot = graph3.0,
       width = 4, height = 4, units = "in")
...
write.csv(list.h06.unique %>% select(ID), file.path(filter.ref.path,"B_10X_samples", "03_HW", "Loc.h06.unique.csv"), 
          row.names = F, quote = F)

write.csv(list.h06.all %>% select(ID), file.path(filter.ref.path, "B_10X_samples", "03_HW","Loc.h06.all.csv"), 
          row.names = F, quote = F)
...
cmd3 <- paste("--vcf", file.path(filter.ref.path, "B_10X_samples/02_MissingData/populations.178074snps.511ind.recode.vcf"), 
              "--recode",
              #paste("--snp", LOC.FILTER$ID[1:2000], collapse = " "),
              
              #"--positions", file.path(get.value("filter.ref.path"),"06c_HeHo_byPlate","Test.tsv"),
              "--snps", file.path(here::here(),filter.ref.path,"B_10X_samples/03_HW", "Loc.h06.unique.csv"),
              
              "--out", file.path(here::here(),filter.ref.path,"B_10X_samples/03_HW", paste0("populations.",list.h06.unique %>% nrow(),"snps.511ind.H06.single"))
)
...
cat(file = file.path(here::here(),filter.ref.path,"B_10X_samples/03_HW","VCFtools_SnpWhiteList.H06.log"),
    "\n", cmd3, "\n",
    A3, # what to put in my file
    append= F, sep = "\n")


cmd4 <- paste("--vcf", file.path(filter.ref.path, "B_10X_samples/02_MissingData/populations.178074snps.511ind.recode.vcf"), 
              "--recode",
              #paste("--snp", LOC.FILTER$ID[1:2000], collapse = " "),
              
              #"--positions", file.path(get.value("filter.ref.path"),"06c_HeHo_byPlate","Test.tsv"),
              "--snps", file.path(here::here(),filter.ref.path,"B_10X_samples/03_HW", "Loc.h06.all.csv"),
              "--out", file.path(here::here(),filter.ref.path,"B_10X_samples/03_HW", paste0("populations.",list.h06.all %>% nrow(),"snps.511.ind.h06.all"))
)
...
if(!file.exists(file.path(filter.ref.path,"B_10X_samples/03_HW", ".gitignore")) ){
  cat("*.vcf", "*.Rdata", "*.RData", "*.data", "!.gitignore", sep = "\n",
      file = file.path(filter.ref.path, "B_10X_samples/03_HW", ".gitignore")) 
}

# rapid PCA
file.path(current.wd,filter.ref.path,"B_10X_samples/03_HW") %>% list.files()

vcf.path <- file.path(filter.ref.path,"B_10X_samples/03_HW", "populations.39253snps.511ind.H06.single.recode.vcf")
...
#     file = here(filter.ref.path, "B_10X_samples", "03_HW", "PCA.Rdata"))


load(  here(filter.ref.path, "B_10X_samples", "03_HW", "PCA.Rdata"))
...
ggsave(filename = file.path(filter.ref.path, "B_10X_samples", "03_HW", "PCA_test.png"),
       plot = gPCA,
...
#load(here(filter.ref.path, "B_10X_samples" ,"04_DP", "gt.Rdata"))
...
save(list = c("gt.tidy", "gt.ind", "gt.key" , "vcf.fix"),
     file = here(filter.ref.path, "B_10X_samples" ,"04_DP", "gt.Rdata"))
...
LOC.DP.max <- gt.key %>% filter(medianDP > 95) %>% pull(ID)

length(LOC.DP.min)
length(LOC.DP.max)

LOC.H06 <- read.csv(file.path(filter.ref.path, "B_10X_samples", "03_HW", "Loc.h06.all.csv"))

LOC.H06.DP <- LOC.H06 %>% filter(ID %nin% c(LOC.DP.min, LOC.DP.max))

nrow(LOC.H06.DP) + length(LOC.DP.min) + length(LOC.DP.max)== nrow(LOC.H06)

write.csv(LOC.H06.DP, file.path(filter.ref.path, "B_10X_samples", "04_DP", "Loc.H06.all.DP.csv"), 
          row.names = F, quote = F)
...
cmd5 <- paste("--vcf", file.path(vcf.path), 
              ...
              #"--positions", file.path(get.value("filter.ref.path"),"06c_HeHo_byPlate","Test.tsv"),
              "--snps", file.path(current.wd,filter.ref.path,"B_10X_samples", "04_DP", "Loc.H06.all.DP.csv"),
              "--out", file.path(current.wd,filter.ref.path,"B_10X_samples", "04_DP", paste0("populations.",LOC.H06.DP %>% nrow(),"snps.511ind.H06.all"))
)
...
cat(file = file.path(current.wd,filter.ref.path,"B_10X_samples", "04_DP","VCFtools_SnpWhiteList.H06.DP.log"),
    "\n", cmd5, "\n",
    A5, # what to put in my file
    append= F, sep = "\n")


if(!file.exists(file.path(filter.ref.path, "B_10X_samples", "04_DP", ".gitignore")) ){
  cat("*.vcf", "!.gitignore", sep = "\n",
      file = file.path(filter.ref.path, "B_10X_samples", "04_DP", ".gitignore")) 
}
...
file.path(current.wd,filter.ref.path, "B_10X_samples", "04_DP") %>% list.files()
...
#     file = here(filter.ref.path, "B_10X_samples", "05_BatchEffect", "rda.Rdata"))


load(here::here(filter.ref.path, "B_10X_samples" ,"05_BatchEffect", "rda.Rdata"))
# Group by individuals
...
# Df Variance      F Pr(>F)
# Model      5     51.2 1.2277  0.117
# Residual 505   4209.9   

RsquareAdj(RDA.plate)$adj.r.squared  #  0.002227419
...
file.path(here::here(),filter.ref.path,"B_10X_samples", "04_DP") %>% list.files()

# In VCF tool, check for duplicated ind.

cmd <- paste("--vcf", file.path(here::here(), filter.ref.path,"B_10X_samples", "04_DP", "populations.174257snps.511ind.H06.all.recode.vcf"), 
             #sub_indv,
             # --depth,
             "--relatedness2"
...
setwd(file.path(filter.ref.path,"B_10X_samples", "06_Relatedness")) 
...
if(!file.exists(file.path(filter.ref.path,"B_10X_samples", "06_Relatedness", ".gitignore")) ){
  cat("out.relatedness2", "!.gitignore", sep = "\n",
      file = file.path(filter.ref.path,"B_10X_samples", "06_Relatedness",  ".gitignore")) 
}

related2.all <- read.delim(file.path(filter.ref.path,"B_10X_samples", "06_Relatedness","out.relatedness2"), header = T, sep = "\t" )
related2.all %>% head()
...
ggsave(filename = file.path(filter.ref.path,"B_10X_samples", "06_Relatedness", "Relatedness.png"),
...
# good.ID  %>% length()
# 
# vcf.path <-  file.path(here::here(),filter.ref.path,"B_10X_samples", "04_DP", "populations.169464snps.553ind.H06.all.recode.vcf")
# 
# cmd2 <- paste("--vcf", vcf.path, 
#               "--recode",
#               paste("--indv", good.ID, collapse = " "),
...
# A2 %>% tail()
# 
# cat(file = file.path(filter.ref.path, "B_10X_samples", "06_Relatedness","VCFtools_RemoveRelatedness25.log"),
#     "\n", cmd2, "\n",
#     A2, # what to put in my file
#     append= F, sep = "\n")
# 
...
vcf.path <-  file.path(here::here(),filter.ref.path,"B_10X_samples", "04_DP", "populations.174257snps.511ind.H06.all.recode.vcf")

cmd2 <- paste("--vcf", vcf.path, 
              "--recode",
              paste("--indv", good.ID.arcto, collapse = " "),
              "--out",  vcf.path %>% str_replace("04_DP", "06_Relatedness") %>% 
                str_replace("511ind", paste0(length(good.ID.arcto), "indwArctogadus")) %>% 
                str_remove(".recode.vcf")
)
...
# vcf.path <- file.path(filter.ref.path, "B_10X_samples", "06_Relatedness", "populations.169464snps.540ind.H06.all.recode.vcf")
# vcf.data <- vcfR::read.vcfR(vcf.path)
# 
# #gl.data  <- vcfR::vcfR2genlight(vcf.data) 
# gi.data  <- vcfR::vcfR2genind(vcf.data) 
...
# 
# write.csv(SCAFFOLD.info.final%>% select(ID), file.path(filter.ref.path,"B_10X_samples", "07_Final", "Loc.H06.DP.unique.final.csv"), 
#           row.names = F, quote = F)
# 
# write.csv(SCAFFOLD.info.final, file.path(filter.ref.path,"B_10X_samples", "07_Final",  "Scaffold.info.H06.DP.unique.final.csv"), 
#           row.names = F, quote = F)
# 
# 
# # CREATE VCF WITH UNIQUE
...
#              "--recode",
#              "--snps", file.path(here::here(),filter.ref.path, "B_10X_samples", "07_Final", "Loc.H06.DP.unique.final.csv"),
#              
#              "--out", file.path(here::here(),filter.ref.path, "B_10X_samples", "07_Final", paste0("populations.", SCAFFOLD.info.final %>% nrow(),"snps.","540ind.H06.DP.single.final"))
# )
# 
# cmd
# 
...
# tail(A)
# 
# cat(file =  file.path(filter.ref.path,  "B_10X_samples", "07_Final", "VCF.filter.log"),
#     "\n", cmd, "\n",
#     A, # what to put in my file
#     append= T, sep = "\n")
# 
...
# 
# vcf.path <- file.path( "./00_Data/06b_Filtering.ref",  "B_10X_samples", "07_Final", "populations.28285snps.540ind.H06.DP.single.final.recode.vcf")
# vcf.data <- vcfR::read.vcfR(vcf.path)
# 
...
#      file = file.path("./00_Data/06b_Filtering.ref", "B_10X_samples", "07_Final", "populations.28285snps.540ind.adegenet.Rdata"))
# 
# 
# if(!file.exists(file.path("./00_Data/06b_Filtering.ref", "B_10X_samples", "07_Final", ".gitignore")) ){
#   cat("*.vcf", "*.Rdata", "!.gitignore", sep = "\n",
#       file = file.path("./00_Data/06b_Filtering.ref", "B_10X_samples", "07_Final", ".gitignore")) 
# }

# Final dataset w ARCTOGADUS -----------------------------------------------------------

vcf.path <- file.path(filter.ref.path, "B_10X_samples", "06_Relatedness", "populations.174257snps.507indwArctogadus.H06.all.recode.vcf")
vcf.data <- vcfR::read.vcfR(vcf.path)
...
write.csv(SCAFFOLD.info.MAF01%>% select(ID), file.path(filter.ref.path,"B_10X_samples", "07_Final", "Loc.H06.DP.unique.final_wArctogadu.csv"), 
          row.names = F, quote = F)

write.csv(SCAFFOLD.info.MAF01, file.path(filter.ref.path,"B_10X_samples", "07_Final",  "Scaffold.info.H06.DP.unique.final_wArctogadus.csv"), 
          row.names = F, quote = F)

SCAFFOLD.info.MAF01 <- read.csv(file.path(filter.ref.path,"B_10X_samples", "07_Final",  "Scaffold.info.H06.DP.unique.final_wArctogadus.csv"))
SCAFFOLD.info.MAF01 %>% nrow()
# CREATE VCF WITH UNIQUE

vcf.path <- "./00_Data/06b_Filtering.ref/B_10X_samples/06_Relatedness/populations.174257snps.507indwArctogadus.H06.all.recode.vcf"

cmd <- paste("--vcf", vcf.path, 
             "--recode",
             "--snps", file.path(here::here(),filter.ref.path, "B_10X_samples", "07_Final", "Loc.H06.DP.unique.final_wArctogadu.csv"),
             
             "--out", file.path(here::here(),filter.ref.path, "B_10X_samples", "07_Final", paste0("populations.", SCAFFOLD.info.MAF01 %>% nrow(),"snps.","507indwArctogadus.H06.DP.single.final"))
)
...
vcf.path <- file.path( "./00_Data/06b_Filtering.ref",  "B_10X_samples", "07_Final", "populations.38131snps.507indwArctogadus.H06.DP.single.final.recode.vcf")
vcf.data <- vcfR::read.vcfR(vcf.path)
...
save(list = c("gl.final", "gi.final"),
     file = file.path("./00_Data/06b_Filtering.ref", "B_10X_samples", "07_Final", "populations.38131snps.507indwArctogadus.adegenet.Rdata"))
