...
#save(list = c("pca.all" ),
#     file = here("02_Results/01_PopStruct", "00_mtDNA", "PCA.Rdata"))

load( here::here("02_Results/01_PopStruct", "00_mtDNA", "PCA.Rdata"))

QuickPop::pca_varplot(pca.all)

Arctogadis.ID <- read_csv("02_Results/01_PopStruct/00_mtDNA/ArctogadusSeq1.csv")

pca.all %>% QuickPop::pca_scoretable(naxe = 6) %>%
  ...
  ggplot(aes(x = score.PC1, y = score.PC2, col = Taxon)) +
